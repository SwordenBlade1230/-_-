...
最後更新時間：2020/10/02
更新內容
1. 修改show_result函式的程式碼，避免出現CT影像沒有和其對應的標記資料的情況
"""
...
def show_result(target_dataset_base_dir, result_num = 5, roi_description = 'roi', roi_name_chinese = '重點區域',
    ...
    anno_name_list = random.sample(os.listdir(os.path.join(target_dataset_base_dir, f'annotations_{roi_description}')), result_num)
    
    for target_img in anno_name_list:
        total_pic_num = 2
        pic_idx = 1
